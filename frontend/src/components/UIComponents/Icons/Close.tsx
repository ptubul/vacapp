...
interface CloseIconProps {
  color: string;
  onClose?: () => void; // פונקציה אופציונלית שתאפשר לשנות את התנהגות הסגירה
}
...
function CloseIcon({ color, onClose }: CloseIconProps) {
  ...
  const handleClose = () => {
    if (onClose) {
      onClose(); // השתמש בפונקציה המותאמת אישית אם היא סופקה
    } else {
      navigate("/"); // ברירת מחדל לנווט לדף הראשי
    }
  };
  ...
  return (
    <svg
      ...
      id="fi_5368396"
      onClick={handleClose} // קריאה לפונקציה שתפעיל את הסגירה
    >
      <path
